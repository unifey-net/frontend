...
const DebugCommunity: React.FC<{ community: string }> = ({ community }) => {
    ...
    const modal = (
        <Modal visible={visible} onCancel={() => setVisible(false)}>
            {status.status === COMPLETE && req !== null && (
                <div>
                    Name: {req.community.name} <br />
                    Permissions: {JSON.stringify(
                        req.community.permissions
                    )}{" "}
                    <br />
                    Self Role: {req.selfRole} <br />
                    Logged In: {self.isLoggedIn ? "YES" : "NO"} <br />
                    Username: {self.user.username} <br />
                    Joined Communities: {JSON.stringify(
                        self.member.member
                    )}{" "}
                    <br />
                    Joined Notification Communities:{" "}
                    {JSON.stringify(self.member.notifications)} <br />
                    Community Supported Emotes: {JSON.stringify(
                        req.emotes
                    )}{" "}
                    <br />
                </div>
    ...
    return (
        <>
            {modal}{" "}
            <DebugCommunityButton onClick={() => setVisible(prev => !prev)}>
                <MdNotificationImportant /> Debug
            </DebugCommunityButton>
        </>
    )
}
